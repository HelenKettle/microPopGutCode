#' pH Limitation Function for microPopGut
#'
...
#'
#' @export
pHLimFuncMPG=function (strainName, groupName, pH, parms){

    pHcorners = parms$strainPHcorners[strainName, ]

    if (is.na(sum(pHcorners))) {
        stop(paste("MICROPOP ERROR: There is an NA in pH corners for group", 
            groupName))
    }
    if (!is.finite(pH)) {
    ...
    if (!all(diff(pHcorners) >= 0)) {
        stop(paste("MICROPOP ERROR: pH corners for group", groupName, 
